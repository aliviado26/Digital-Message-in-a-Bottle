// Client-side mirror of supabase/functions/_shared/current-systems.ts, for
// drawing the current-flow overlay on the Explore map. The Edge Function's
// copy is the one that actually drives simulation — this one only needs to
// look right, so keep them in sync by eye rather than sharing a module
// (the Edge Function runs on Deno with npm:/relative-.ts imports that don't
// resolve the same way under Next's bundler).
...
export function getCurrentVelocity(position: LatLng): { u: number; v: number; speedMs: number; headingDeg: number } {
  ...
  const finalU = u / totalWeight;
  const finalV = v / totalWeight;
  const speedMs = Math.sqrt(finalU * finalU + finalV * finalV);
  const headingDeg = (Math.atan2(finalU, finalV) * 180) / Math.PI;

  return { u: finalU, v: finalV, speedMs, headingDeg: (headingDeg + 360) % 360 };
}
